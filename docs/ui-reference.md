...
1. **Header** — purple bar, app name, live count of printers and jobs (rows
   ...
   **build stamp**, the **view toggle**, and a gear opening **Shop layout**
   ...
   The **build stamp** reads `build <version>` after the counts, and repeats
   on the sign-in screen so a tab that never loads the board can still report
   which code it is running. It comes from the `BUILD` constant at the top of
   `print-farm-scheduler.jsx`, which is **bumped by hand in every commit that
   changes that file**. (The Teams manifest version used to be stamped beside
   it; it was a second hand-maintained number that drifted from the package,
   and the Developer Portal already answers that question.) Compare it against the version that was merged to tell a
